...
assert_exit 1 "$(run --envelope-path="$E9")" "empty S1.Q3 interpretation fails"

echo "[validate-envelope.test] Missing prose section -> exit 1"
E10="$TMPDIR/no-section.md"; make_valid_envelope "$E10"
sed -i '/^## S3 Composite intent envelope$/d' "$E10"
assert_exit 1 "$(run --envelope-path="$E10")" "missing S3 section fails"
