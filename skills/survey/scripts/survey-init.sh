...
# survey envelope into an output directory, optionally seeding the S0 Context with
...
#   --item-text-file optional; file whose contents seed S0 Context. Missing/empty ->
...
# Seed S0 Context with the work-item text the operator provided (happy path).
if [[ -n "$ITEM_TEXT_FILE" && -s "$ITEM_TEXT_FILE" ]]; then
  ...
  echo "[survey-init] scaffolded $ENVELOPE_PATH (no work-item text provided; S0 Context placeholder retained for manual fill)"
fi
